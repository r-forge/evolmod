...
\title{An rbrothers Demo}
\maketitle

This is a demo for using the java program, DualBrothers, in R.  To
get started, install the rbrothers R package from the directory that contains the rbrothers\_0.0-1.tar.gz file. The rJava package will need to be installed.
\begin{verbatim}
