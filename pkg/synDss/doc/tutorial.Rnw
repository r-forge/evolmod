...
<<label=install>>=
# pickup here
data.salm<-as.matrix.alignment(ape::as.alignment(read.fasta(system.file("seqdata/Salmonella_fimH_aligned-18alleles.txt",package="synDss"),forceDNAtolower=F)))
align.taxa<-nuc.to.codon(data.salm)
