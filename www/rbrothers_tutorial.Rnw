...
This is a tutorial demonstrating the usage of for using the java program, DualBrothers, in R.  To get started, install the rbrothers R package from R-forge. 
\begin{verbatim}
...
\begin{figure}[!t]
  \centering
<<label=dbfigure1,fig=TRUE,echo=FALSE, width=6,height=4.0>>=
<<plotdb>>
@
